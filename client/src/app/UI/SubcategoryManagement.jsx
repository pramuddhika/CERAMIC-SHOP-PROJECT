...
const SubcategoryManagement = () => {
  ...
  const getNewId = async () => {
    try {
      const Idresponse = await axios.get(`/api/productdata/get/subcategory`);
  ...
  const fetchSubcategoryData = async (page, limit) => {
    setIsLoading(true);
    try {
      const response = await axios.get(
        `/api/productdata/get/tableData/subcategory?page=${page}&limit=${limit}`
      );
  ...
  useEffect(() => {
    getNewId();
    fetchCategoryList();
    fetchSubcategoryData(currentPage, itemsPerPage);
